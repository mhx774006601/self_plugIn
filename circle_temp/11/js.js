 $(function(){
	        ...
	        var min=2010;
	        //整个圆盘所有的显示刻度
	        var scale = (max-min)*12+1;  //61
	        //根据最大值与最小值的差，算出不同数据展示所需要的每一个刻度的角度。
	        var angles = 300/(scale-1);  //5
	         //画所有刻度
	        ...
	        //再对所画的所有刻度进行角度旋转
	        for(var i=1;i<=scale;i++){
	        	$('#list li').eq(i).addClass('lis').css({'transform':'rotate('+(210 + i*angles)+'deg)'});
	        }
	       ...
	       //创建大刻度值并进行角度旋转
          	for(var i = 0;i<nodes;i++){
          		$('#tex span').eq(i).css({'transform':'rotate('+(210 + i*Ang)+'deg)'});
          	}
